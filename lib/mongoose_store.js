function incr(key, lifeTimeMs = 6000, Model, callback){
    ...
    model.findOneAndUpdate({ip: key}, actions, {new: true}, (err, doc) => {
        ...
        if(!doc){ 
            ...
            newKey.ip = key;
            newKey.counter = 1;
            newKey.expirationDate = expiresAt;
...
function decrement(key, Model){
    const expiresAt = new Date(Date.now() + lifeTimeMs);
    const actions = {$inc: {counter: -1}, $set: {expirationDate: expiresAt}};
    Model.findOneAndUpdate({ip: key}, actions, {new: true}, (err, doc) => {
        if(err) return console.log('MongooseStore: Error decrementing document:', err);
        if(!doc) return console.log('MongooseStore: No document found for decrement...');
        console.log('MongooseStore: Document decremented successfully.')
    })  
}

function resetKey(key, Model){
    Model.deleteOne({ip: key}, (err) => {
        if(err) return console.log('MongooseStore: Error resetting document:', err);
